#!/bin/sh
# Script to run the MEA to design guides that perform SNP identification and differentiate between viral lineages
...
# 'ver2' refers to the fact that the initial set of input sequences did not include all the VOIs, so this one does 
# All the sequences for the different lineages are in the same .fasta file, so we use the option  --all_targets_one_file
python design_guides.py diff both './alignments/full-genome-COV2-ver2/final alignment VOI.fasta' ./gen_results/full-genome-COV2-ver2/ --num_cpu 80 --verbose_results --save_to_pickle --output_to_single_directory